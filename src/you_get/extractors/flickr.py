...
pattern_url_group = 'https?://www.flickr.com/groups/([^/]+)'
...
# used for getting video source
tmpl_photo_page = 'https://www.flickr.com/photos/%s/%s';
...
# looks that flickr won't return urls for all sizes we required in 'extras field without a acceptable header
dummy_header = {
...
def get_photo_id(url, page):
    return match1(url, pattern_url_photo)
...
url_patterns = [
    ...
    # according to flickr api documentation, this method needs to be authenticated in order to filter photo visible to the calling user, but it seems works fine anonymously as well
    'people.getPhotos',
    'user_id',
    get_NSID,
    'photos'
...
def flickr_download_main(url, output_dir='.', merge = False, info_only = False, **kwargs):
    ...
    if match1(url, pattern_url_single_photo):
        ...
        urls, title = fetch_photo_url_list(url, info_only, size)
        print (title)
    serial = 0;
    for url in urls:
        ...
        if not info_only:
            download_urls([url], title + str(serial), ext, False, output_dir, None, False, False)
            serial = serial + 1
    

# url dispatcher
def fetch_photo_url_list(url, info_only, size):
    id = None
    for pattern in url_patterns:
        # TODO: fix multiple matching since the match group is dropped
...
def get_orig_video_source(info):
...
def get_url_of_largest(info, api_key, size):
    if info['media'] == 'photo':
        ...
        parsed = json.loads(get_content_headered(tmpl_api_call_video_info % (api_key, info['id'], info['secret'])))
        return get_orig_video_source(parsed).replace('\\', '')

def get_single_photo_url(url):
    page = get_html(url);
    match = match1(page, pattern_inline_img_url)
    #last match always has the best resolution
    return 'https:' + match.replace('\\', ''), match1(page, pattern_inline_title)
